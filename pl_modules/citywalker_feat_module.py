...
class CityWalkerFeatModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        wp_pred, feature_pred, feature_gt = self(obs, cord, future_obs)
        losses = self.compute_loss(wp_pred, feature_pred, feature_gt, batch)
        ...
        total_loss = waypoints_loss + self.direction_loss_weight * direction_loss + feature_loss * self.feature_loss_weight
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.datatype == "citywalk":
            wp_pred, _, _ = self(obs, cord, future_obs)
    ...
    def compute_loss(self, wp_pred, feature_pred, feature_gt, batch):
        waypoints_target = batch['waypoints']
        ...
        return {'waypoints_loss': wp_loss, 'direction_loss': direction_loss, 'feature_loss': feature_loss}
    
    def compute_loss_polar(self, wp_pred_euclidean, distance_pred, angle_pred, batch):
        waypoints_target = batch['waypoints']
        '''
        arrived_target = batch['arrived']
        '''
        ...
        return {'distance_loss': distance_loss, 'angle_loss': angle_loss, 'direction_loss': direction_loss}
