...
class CityWalkerCropModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        assert self.decoder == "diff_policy"
        wp_pred, noise_pred, noise = self(obs, cord, batch['waypoints'])
        losses = self.compute_loss_diff_policy(wp_pred, noise_pred, noise, batch)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        wp_preds = self.model.sample(obs, cord, num_samples=10) * batch['step_scale'].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    ...
    def compute_loss_diff_policy(self, wp_pred, noise_pred, noise, batch):
        ...
        return {'noise_loss': noise_loss, 'direction_loss': direction_loss}
        ...
        for idx in range(batch_size):
            ...
            if pred_waypoints.ndim == 3:
                ...
                for sample_idx in range(pred_waypoints.shape[0]):
                    ...
                    u_pred, v_pred = translate(u_pred, v_pred)
                    if np.all(valid):
                ...
                u_pred, v_pred = translate(u_pred, v_pred)
                if np.all(valid):
                    ax1.plot(u_pred[valid], v_pred[valid], 's-', color='#DB6057')
            else:
                raise ValueError
            ...
            if pred_waypoints.ndim == 3:
                ...
                for sample_idx in range(pred_waypoints.shape[0]):
                    ...
                    ax2.plot(pred_waypoints[sample_idx, :, 0], pred_waypoints[sample_idx, :, 1],
                        's-', label=label, color='#DB6057', alpha=0.8)
            elif pred_waypoints.ndim == 2:
