#!/usr/bin/env python3
"""
Training script for the toy flow-matching obstacle-bypass experiment.

Trains a FiLM-conditioned MLP velocity field to generate multi-modal
bypass trajectories conditioned on raycasting distances.  Produces
visualisation figures throughout training.

Usage (from project root or from within flow_matching_test/):
    python flow_matching_test/train.py [OPTIONS]
    # or
    cd flow_matching_test && python train.py [OPTIONS]

Requires: torch, numpy, matplotlib  (no other dependencies)
"""
...
# --------------------------------------------------------------------------- #
# Visualisation helpers (v2 – occupancy maps, roads, endpoint markers)
# --------------------------------------------------------------------------- #
...
def visualize_comparison(model, eval_data, device, save_path,
    ...
    for col, d in enumerate(eval_data):
        ...
        _setup_panel(ax, f"Model: {d['label']}")

    fig.suptitle(title if title else "Model Samples", fontsize=13)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def main():
    parser = argparse.ArgumentParser(
        description="Toy conditional flow matching: obstacle bypass")
    # --- dataset ---
    ...
    print(f"Device: {device} | Results → {args.output_dir}")
    ...
    cfg_str = f'n_scenes={args.n_scenes}|epochs={args.epochs}|n_rays={n_rays}|hidden_dim={args.hidden_dim}'
    ...
    if not args.no_vis:
        ...
        model.load_state_dict(
            torch.load(os.path.join(args.output_dir, "best_model.pt"),
                       weights_only=True))
        '''
