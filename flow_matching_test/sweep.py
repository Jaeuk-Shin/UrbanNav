#!/usr/bin/env python3
"""
Hyperparameter sweep for flow_matching_test/train.py.
...
Usage:
    python flow_matching_test/sweep.py
    python flow_matching_test/sweep.py --jobs_per_gpu 2 --num_gpus 4
    python flow_matching_test/sweep.py --plot_only   # skip training, just re-plot
"""
...
DEFAULT_SWEEP_DIR = os.path.join(SCRIPT_DIR, "sweep_results")
...
def plot_val_loss_grid(results, sweep_dir):
    ...
    fig.suptitle("Validation Loss — Hyperparameter Sweep", fontsize=14, y=1.02)
    plt.tight_layout()
...
def plot_best_val_summary(results, sweep_dir):
    ...
    fig.suptitle("Best Validation Loss vs. n_scenes", fontsize=14, y=1.02)
    plt.tight_layout()
