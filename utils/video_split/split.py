...
def split_segment_wrapper(segment_info):
    """Unpacks the tuple and calls the splitting function."""
    return split_segment(*segment_info)

def split_segment(input_path, output_path, start_time, segment_duration):
    """
    Split a single segment using FFmpeg Stream Copy.
    """
    if os.path.exists(output_path):
        return

    # Optimization: -ss before -i for fast seeking
    # Optimization: -c copy to avoid re-encoding
    cmd_split = [
        'ffmpeg',
        '-y',
        '-ss', str(start_time), 
        ...
        '-c', 'copy',          
        '-map', '0:v',         # Explicitly map only video stream 0
        '-an',                 # Discard audio
        '-loglevel', 'error',  # Keep stderr clean for tqdm
        output_path
    ]
    ...
    try:
        ...
        print(f"Error at {start_time}s in {input_path}: {e}")
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        # list() forces the generator to execute
        list(tqdm(executor.map(split_segment_wrapper, all_segments), 
