...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, cfg, port=2000, max_speed=1.4, fps=5, gamma=0.99,
                 towns=None, randomize_weather=False):
        ...
        self.action_space = gym.spaces.Box(low=-100., high=100., shape=(self.future_length*2,), dtype=np.float32)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        settings.max_substep_delta_time = self.dt / settings.max_substeps
    ...
    def step(self, action: np.ndarray):
        ...
        """
        assert isinstance(action, np.ndarray) and action.shape == (self.future_length*2,)
        self._substep_frames = []
        ...
        waypoints_cam = np.reshape(action, (-1, 2))   # (N, 2) in camera frame: (x_cam, z_cam)

        # Convert camera-frame waypoints to absolute world (x, z) positions
        # so that substep updates account for both translation AND rotation.
        c2w_R0 = R.from_quat(self.pose[3:])
        xz0 = self.xz
        wp_3d_cam = np.column_stack([
            waypoints_cam[:, 0],
            np.zeros(len(waypoints_cam)),
            waypoints_cam[:, 1],
        ])
        wp_3d_world = c2w_R0.apply(wp_3d_cam)
        waypoints_world = wp_3d_world[:, [0, 2]] + xz0   # absolute world (x, z)

        for t in range(self.n_skips):
            # Re-project world waypoints into the current camera frame
            c2w_R = R.from_quat(self.pose[3:])
            wp_rel = waypoints_world - self.xz             # relative, world frame
            wp_3d_rel = np.column_stack([
                wp_rel[:, 0],
                np.zeros(len(wp_rel)),
                wp_rel[:, 1],
            ])
            wp_cam_3d = c2w_R.inv().apply(wp_3d_rel)       # world → camera
            waypoints = wp_cam_3d[:, [0, 2]]                # (x_cam, z_cam)
            ...
            processed_waypoints = repeat_and_shift(data=waypoints, repeats=self.n_skips, shifts=t)
            cost_weights = np.squeeze(repeat_and_shift(data=cost_weights[:, None], repeats=self.n_skips, shifts=t), axis=-1)

            x, u, mpc_stats = self.mpc.solve(
                initial_pose=np.array([0., 0., .5*np.pi]),      # (x, z, th)
                waypoints=processed_waypoints,
                cost_weights=cost_weights
                )

            walker_control = to_walker_control(u[0], c2w=self.pose, dt=self.dt)
            self.robot.apply_control(walker_control)
            self.world.tick()
            self.update_buffer()
            if self._collect_substep_frames:
                self._substep_frames.append(self.rgb_buffer[-1].copy())
        self.path_length += float(np.linalg.norm(self.xz - pre_step_xz))
        ...
        reward = 3. if self.distance_to_goal <= .2 else 0.
        # normalization
        reward += (self.gamma * phi1 - phi0) / (self.gamma * self.n_skips * self.dt * self.max_speed)
        
        # reward = -np.sum(self.goal ** 2) ** .5       # ell^2 distance toward the goal
        info = self._get_info()
...
def to_walker_control(velocities, c2w, dt) -> carla.WalkerControl:
    """
    ...
    ctrl = carla.WalkerControl(direction, float(lin_v))   # m -> cm
