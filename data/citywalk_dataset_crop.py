...
class CityWalkCropDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Process frames
        frames = self.process_frames_wrt_vanishing_point(frames)
    ...
    def process_frames_wrt_vanishing_point(self, frames):
        """
        WARNING! not general enough to cover all cases!
        """
        frames = torch.tensor(frames).permute(0, 3, 1, 2).float() / 255.0  # Corrected normalization
        ...
        X_VANISHING = int(431.2315218029843)     # x-coordinate of the vanishing point
        left = 2 * X_VANISHING - W
        top = 120
        frames = TF.crop(frames, top=top, left=left, height=360, width=W-left)
